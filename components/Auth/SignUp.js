...
export default function SignUp({setauthpage,signup,error}) {
    ...
    const checkvalid = () =>{   // validation for signup fields
            let obj = {email:false,pass:false,name:false}
            if(validator.isEmail(email)){
                setemailerr("");
                obj.email =true;
            }else setemailerr("Invalid Email");
            if(!validator.isEmpty(name)){
                setnameerr("");
                obj.name = true;
            }else setnameerr("Required");
            if(validator.matches(password,/^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])/) && validator.isLength(password,{ min:8, max:12})){
                setpasserr("");
                obj.pass = true;
            }else setpasserr("Password must contain a number, uppercase / lowercase alphabet & minimum 8 charecters");
        
            if(obj.pass && obj.email && obj.name) {
    ...
    return ( //signup Page
        ...
        <div className="hidden md:block">
            <Image alt="image"  width={350} height={540} className="max-width-full max-height-full" src='/assets/authpage.png' />
        </div>
