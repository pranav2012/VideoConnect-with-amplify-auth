...
export default function ConfirmUser({email,confirmuser,error}) {
    
    const [otp, setotp] = useState("");
    const [invaliderr, setinvaliderr] = useState("");

    const checkvalid = () =>{ //validation for OTP
        if(validator.isInt(otp) && validator.isLength(otp,{ min: 6, max:6 })){
            confirmuser(email, otp);
            setinvaliderr("");
        }else setinvaliderr("Incorrect Otp");
    }
    
    return ( //user validation promt with otp (email)
        <div className="flex w-full">
        <div className="hidden md:block">
            <Image objectFit alt="image" width={350} height={500} src='/assets/authpagetwo.png' />
        </div>
        <div className="p-4 flex-1">
            ...
            <p className="text-gray-400 text-sm mt-4 text-center">Your verification code is sent to your mail please check your mail. </p>
            <input onChange={(e) => setotp(e.target.value)} value={otp} className="mt-24 w-full px-3 py-3 placeholder-blueGray-300 text-blueGray-600 relative bg-whiterounded text-sm border border-blueGray-300 outline-none focus:outline-none focus:ring pr-10" type="text" name="otp" placeholder="Verification Code" />
            <p className="text-center text-red-600 text-xs">{invaliderr}</p>
            <br/>
            ...
            <div onClick={() => { checkvalid() }} className="mt-8 cursor-pointer mx-auto w-3/4 bg-red-700 p-1 px-2 rounded">
                <p className="ml-2 text-white text-center text-sm">Confirm Code</p>
            </div>
        </div>
    </div>
    )
